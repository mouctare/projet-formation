...
useEffect(() => {
 ...
 const handleSubmit = async event => {
     ...
     if(!planning.dateStart) 
      ...
      if(!planning.user) 
        apiErros.user = 
       "L'agent à qui appartient  planning  est obligatiore";

       if(!planning.site) 
        apiErros.site = 
        "Le site lié au planning  de l'agent est obligatiore";
        
        setErrors(apiErros);
        if(editing){
          if(Object.keys(apiErros).length===0){
        await PlanningsAPI.update(id, planning).then((data)=> {
          setErrors({});         
          ...
          if(Object.keys(apiErros).length===0){
        await PlanningsAPI.create(planning).then((data)=> {
