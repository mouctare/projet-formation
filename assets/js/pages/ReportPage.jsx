...
const ReportPage = ({ history }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const apiErros = {};
    if (!report.title) apiErros.title = "Le titre du rapport est obligatiore";
    if (!report.description)
      apiErros.description = "Le rapport doit avoir un minumum de description";
    if (!report.site)
      apiErros.site = "Le site  auquel est lié le rapport   est obligatiore";
    setErrors(apiErros);

    if (Object.keys(apiErros).length === 0) {
      await ReportsAPI.create(report)
        .then((data) => {
          toast.success("Le rapport a bien été crée");
