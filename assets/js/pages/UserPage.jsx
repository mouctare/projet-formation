...
import FormContentLoader from "../components/loaders/FormContentLoader";
 ...
 const [editing, setEditing] =  useState(false);
    ...
    const fetchUser = async id => {
         try {
            const {email,password,  firstName, lastName, roles,  cardPro, dateCreatedCarPro, expiryDateCardPro } =  await UsersAPI.find(id);
               setUser({ email,password,  firstName, lastName, roles,  cardPro, dateCreatedCarPro, expiryDateCardPro });
               setLoading(false);
              } catch(error) {
 ...
 useEffect(() => {
   ...
   const handleSubmit = async event => {
       ...
       if(!user.email) 
        apiErros.email = 
          "L'adresse email de l'agent est obligatiore";
        if(!user.lastName) 
            apiErros.lastName ="Le nom de l'agent est obligatoire";
       if(!user.password) 
            apiErros.password =
             "Le mot de passe de l'agent est obligatiore";
       if(!user.firstName) 
          apiErros.firstName =
           "Le prénom de l'agent est obligatoire";
        if(!user.cardPro || user.cardPro.length <  25) 
        apiErros.cardPro =
         ...
         if(editing)
            {  
             if(Object.keys(apiErros).length ===0){
                await UsersAPI.update(id, user).then((data)=> {
                    //setErrors({});
                ...
                if(Object.keys(apiErros).length===0){
                await UsersAPI.create(user).then((data)=> {
       ...
       {!loading && ( 
           <form onSubmit={handleSubmit}>
            ...
            label="Carte professionnelle"
             ...
             value={user.cardPro}
             onChange={handleChange}
             required pattern="\CAR\-\d{3}\-((\d{2}(([02468][048])|([13579][26]))[\/\s-]?((((0?[13578])|(1[02]))[\/\s-]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\/\s-]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\/\s-]?((0?[1-9])|([1-2][0-9])))))|(\d{2}(([02468][1235679])|([13579][01345789]))[\/\s-]?((((0?[13578])|(1[02]))[\/\s-]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\/\s-]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\/\s-]?((0?[1-9])|(1[0-9])|(2[0-8]))))))(\s(((0?[1-9])|(1[0-2])):([0-5][0-9])((\s)|(:([0-5][0-9])\s))([AM|PM|am|pm]{2,2})))?\-\d{11}"   />
            <Field 
           ...
           <div className="form-group">
               ...
               <Link to="/agents" className="btn btn-link">Retour à la liste</Link>
        </div>
        </form> 
        )}
        </>
